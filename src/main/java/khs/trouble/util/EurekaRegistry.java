...
public class EurekaRegistry {
	...
	private static Logger LOG = Logger
			.getLogger(EurekaRegistry.class.getName());
	...
	private void waitForRegistrationWithEureka(EurekaClient eurekaClient) {
		...
		while (nextServerInfo == null) {
			try {
				nextServerInfo = eurekaClient.getNextServerFromEureka(
						vipAddress, false);
			} catch (Throwable e) {
				LOG.info("Waiting ... verifying service " + vipAddress
	...
	private void processRequest(final Socket s) {
		try {
			...
			if (line != null) {
				System.out
						.println("Received a request from the example client: "
								+ line);
			}
	...
	public String discoverAddress(String vipAddress, boolean validate) {
		...
		if (applicationInfoManager == null || eurekaClient == null
				|| configInstance == null) {
		...
		InstanceInfo nextServerInfo = null;
		try {
			nextServerInfo = this.eurekaClient.getNextServerFromEureka(
					vipAddress, false);
		} catch (Exception e) {
			LOG.severe("Cannot get instance " + vipAddress
					+ " to talk to from eureka");
			throw new RuntimeException("Error trying to discover " + vipAddress
					+ e);

		}
		LOG.info("Found an instance of " + vipAddress
				+ "  service to talk to from eureka: "
		...
		if (validate) {
			...
			try {
				...
				System.err.println("Could not connect to the server :"
						+ nextServerInfo.getHostName() + " at port "
						+ serverPort);
			} catch (Exception e) {
				System.err.println("Could not connect to the server :"
						+ nextServerInfo.getHostName() + " at port "
